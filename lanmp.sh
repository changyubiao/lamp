#!/bin/bash
# centos6 ，不区分32位，64位， 要求机器可以上外网。 支持lamp 和 lnmp，
# mysql支持5.1和5.6两个版本，php支持5.3和5.6两个版本，apache2.2，nginx1.8
#
# author  : changyubiao
# date:2016年 08月 25日 星期日 04:59:18  
#
#
...
do
    case $mysql_v in
        ...
        5.6)
            cd /usr/local/src
            [ -f mysql-5.6.33-linux-glibc2.5-$ar.tar.gz ] ||
	     wget http://mirrors.sohu.com/mysql/MySQL-5.6/mysql-5.6.33-linux-glibc2.5-$ar.tar.gz
#             wget http://mirrors.sohu.com/mysql/MySQL-5.6/mysql-5.6.26-linux-glibc2.5-$ar.tar.gz
            tar zvxf mysql-5.6.33-linux-glibc2.5-$ar.tar.gz
            check_ok
            [ -d /usr/local/mysql ] && /bin/mv /usr/local/mysql /usr/local/mysql_bak
            mv mysql-5.6.33-linux-glibc2.5-$ar /usr/local/mysql
            if ! grep '^mysql:' /etc/passwd
            then
                useradd -M mysql -s /sbin/nologin
            fi
